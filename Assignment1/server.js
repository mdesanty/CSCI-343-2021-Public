...
{
  let queryGrades = query["grades"] instanceof Array ? query["grades"] : [query["grades"]];

  if(!queryGrades || queryGrades.length < 1) { throw Error("At least one grades value is required."); }
  queryGrades.map(i => {if(isNaN(i)) { throw Error("All grades nust be numbers.") }});

  let grades = queryGrades.map(i => parseInt(i));
